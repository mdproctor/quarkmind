...
Code review found three synchronisation gaps. `reset()`, `snapshot()`, and `spawnEnemyUnit()` all needed `synchronized`. The `snapshot()` problem was the subtle one: five separate volatile field reads are individually safe but not atomic as a group. A concurrent `tick()` between any two reads produces an inconsistent snapshot. Tests pass single-threaded. Production would have failed quietly.
...
The implementer subagent discovered this by reading the CaseHub source during Task 8. It's not documented anywhere in CaseHub yet — it went straight to the knowledge garden. The four pass-through plugins all carry `@CaseType("starcraft-game")` now.
